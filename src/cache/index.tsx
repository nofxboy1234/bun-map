...
export class SimpleCache {
  ...
  async fetch<T>(key: string, fetcher: () => Promise<T>, options?: CacheFetchOptions): Promise<T> {
    ...
    const retryPolicy: RetryPolicy = {
      retry: Math.max(options?.retry ?? 1, 0),
      retryDelayMs: Math.max(options?.retryDelayMs ?? 300, 0),
    };
    const dedupeMode = options?.dedupeMode ?? "signalAware";
    const now = Date.now();
    const entry = this.getEntry(key);
    ...
    if (entry) {
      ...
      void this.fetchFresh(key, fetcher, {
        staleTime,
        gcTime,
  ...
  private async fetchFresh<T>(
    ...
    if (pending) {
      if (pending.signal?.aborted) {
        this.pending.delete(key);
      } else if (options.dedupeMode === "byKey") {
